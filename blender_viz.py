...
project_folder = r"C:\Users\plank\Documents\git\blender-python-path-render" #TODO remove hard code path
project_path = Path(project_folder)
place = "Freiberg"
project_data_path = Path(project_folder, "data", place)

nodes_df = pd.read_csv(project_data_path / "nodes_transformed.csv", index_col="node_id")
...
bpy.context.scene.render.image_settings.file_format = 'FFMPEG'  # Use FFMPEG video format
...
while algorithm.step():
    ...
    if iteration % nth_iteration == 0:
        ...
        for node_id, fac in node_colors.items():
            color = (0.2745 - (0.2745 - 0.1137) * fac, 0.4666 + (0.8588 - 0.4666) * fac, 0.4823 + (0.4862 - 0.4823) * fac, 0.2 + (1.0 - 0.2) * fac)
            for edge in adj_list[node_id].keys():
                ...
                gp_stroke.end_cap_mode = 'ROUND'
                gp_stroke.points.add(count=2)
                gp_stroke.points[0].co = (vertices[node_id][0], vertices[node_id][1] , vertices[node_id][2] + 0.1)
                gp_stroke.points[0].vertex_color = color
                gp_stroke.points[1].co = (vertices[edge][0], vertices[edge][1] , vertices[edge][2] + 0.1)
...
for i in range(len(path)-1):
    ...
    gp_stroke.points[1].co = (vertices[next_node_id][0], vertices[next_node_id][1], vertices[next_node_id][2] + 0.1)
